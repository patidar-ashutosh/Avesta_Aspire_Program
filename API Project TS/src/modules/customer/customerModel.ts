...
const customerQuery = {
    getMovieByCityNameQuery : async (cityName:string) : Promise<any> => {
    ...
    getMovieByCinemaHallQuery : async (cinemaHallName:string) : Promise<any> => {
    ...
    getMovieByNameQuery : async (movieName:string) : Promise<any> => {
        const [result] = await pool.query("SELECT * FROM movie WHERE name LIKE ?", [`%${movieName}%`]);
        return result;
    },

    getShowSeatingPlanQuery : async (data : {cityName:string, movieName:string, cinemaName:string, cinemaHallName:string, showId:string}) : Promise<any> => {
    ...
    getTopTenActorWithMaxMoviesQuery : async () : Promise<any> => {
    ...
    getMovieByYearQuery : async () : Promise<any> => {
